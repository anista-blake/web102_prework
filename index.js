...
const numGameCards = GAMES_JSON.reduce( (acc, Game) => {
    return acc + 1;
}, 0);

gamesCard.innerHTML = numGameCards.toLocaleString('en-US');
...
let displayStr = 
numUnfunded > 1 ? `A total of $${totalAmountRaised.toLocaleString('en-US')} has been raised for ${numGameCards} games. Currently, 
            ${numUnfunded} games remain unfunded. We need your help to fund these amazing games!` : 
            `A total of $${totalAmountRaised} has been raised for ${numGameCards} games. Currently, 1 game remains 
...
top.innerHTML = `<p> ${first.name} </p>`;
firstGameContainer.appendChild(top);

// do the same for the runner up item
const runnerUp = document.createElement('runner-up');
runnerUp.innerHTML = `<p> ${second.name} </p>`;
